...
RETRIEVAL_QUERY_MODEL_PATH = os.path.join(os.getcwd(), "ReSys/models/retrieval-c2p-query-model")
RETRIEVAL_CAN_MODEL_PATH = os.path.join(os.getcwd(), "ReSys/models/retrieval-c2p-candidate-model")
...
query_model = tf.saved_model.load(RETRIEVAL_QUERY_MODEL_PATH)
candidate_model = tf.saved_model.load(RETRIEVAL_CAN_MODEL_PATH)
...
def get_c2p_recommendations():
    try:
        ...
        ## Create a BruteForce index using the SavedModel query_model
        index = tfrs.layers.factorized_top_k.BruteForce(query_model)
        ## Use the SavedModel candidate_model to map products to embeddings
        index.index_from_dataset(
            tf.data.Dataset.zip((products.batch(2), products.batch(2).map(candidate_model)))
        )
        ## Get recommendations
        _, titles = index(tf.constant([customer_id]))

        product_candidates = [t.decode("utf-8") for t in titles[0, :NUM_OF_CANDIDATES].numpy()]
        ranking_queries = [
            {"customer_id": u, "product_id": m}
            for (u, m) in zip([customer_id] * NUM_OF_CANDIDATES, product_candidates)
        ]
        products_scores = []
        for product_id in product_candidates:
            products_scores.append(ranking_model({"customer_id": np.array([customer_id]), "product_id": [product_id]}).numpy()[0][0])
        ranked_products = [
            m[1] for m in sorted(list(zip(products_scores, product_candidates)), reverse=True)
        ]
        return make_response(jsonify({"product_ids": ranked_products}), 200)
    except Exception as e:
        return make_response(jsonify({"error": f"Recommending failed because {e}"}), 500)
